...
class Task:
    def __init__(self, 
        ...
        if self.name.startswith("test_"):
            warnings.warn(f"Task '{self.name}' may shadow pytest test function collection", stacklevel=2)
...
def task(_func=None, *, name=None, retries=0, store_result=True, before_job: Optional[Callable[[], None]] = None, after_job: Optional[Callable[[Any], None]] = None, on_error: Optional[Callable[[Exception], None]] = None):
    def wrapper(func):
        return Task(func, name=name, retries=retries, store_result=store_result, before_job = before_job, after_job = after_job, on_error = on_error,)
